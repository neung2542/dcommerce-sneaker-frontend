            ...
            <Link key={index} href={shoe.category_id == "1" ? `/launch/${shoe.id}` : `/upcoming/${shoe.id}`}>
              ...
              <div className="mt-8 mb-12 relative">
                <div className="w-3/4 pl-3">
                  <p className="text-2xl text-gray-500">{shoe.sub_desc}</p>
                  <h2 className="text-3xl font-semibold">{shoe.name}</h2>
                </div>
                <div className="absolute right-0 top-1/3">
                  {shoe.category_id == "1" ? (
                    <button className="bg-black text-white rounded-full px-6 py-2 hover:bg-gray-800 text-xl">
                      Buy
                    </button>
                  ) : (
                    <button
                      className="bg-gray-300 text-gray-600 rounded-full px-6 py-2 pointer-events-none text-xl"
                      disabled
                    >
                      Soon
                    </button>
